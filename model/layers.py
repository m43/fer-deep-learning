...
from utils.util import relu, softmax
...
class Convolution(Layer):
    ...
    def forward(self, x):
        ...
        return out.transpose(3, 0, 1, 2)  # TODO why like this?

    def backward_inputs(self, grad_out):
        # nice trick from CS231n, backward pass can be done with just matrix mul and col2im  # TODO check CS231n
...
class FC(Layer):
    ...
    def forward(self, inputs):
        ...
        """
        self.inputs = inputs
        out = self.inputs @ self.weights.T + self.bias
        return out
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y):
        ...
        """
        self.logits = softmax(x)
        self.N = x.shape[0]
        out = - np.sum(y * np.log(self.logits)) / self.N  # might be implemented faster
        return out
